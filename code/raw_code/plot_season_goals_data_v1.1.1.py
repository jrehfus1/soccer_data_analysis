...
#summary of last modification: added comments to facilitate readability

################################################################################
##### description #####
#this script plots a single figure with the following three subplots:
#   1) histogram of the all of the goal times for each goal scored in a PL season
#   2) histogram of the total number of goals scored in all of the matches in a PL season
#   3) histogram of the final goal difference for all of the matches in a PL season
...
axes[0].set_ylim(0, max(goal_times_bin_counts)+5)
...
axes[2].plot([avg_victory_margin_per_match, avg_victory_margin_per_match], [0,380], linewidth=2.0, linestyle=':', color=(0.07,0.41,0.98)) #plot a line showing the average victory margin in a match
...
if save_fig: #save the figure if you like
    figure_name = season_span + '_PL_goals_figure.pdf'
    plt.savefig( out_path_figs + figure_name, dpi=150, format=None, transparent=True ) #facecolor='w', edgecolor='w')
    print('Saved figure.')
