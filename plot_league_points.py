#this code plots the league points for each premier league team as a function of week
...
season_span='19_20'
in_file='data/' + season_span + '_PL_goals_web_data.csv'
match_results_df=pd.read_csv(in_file)

##### make an empty dataframe for league score for each team for each match week #####
#this will have 38 rows, and 20 columns, and contain the number of league points that each team won each match week
save_mw_league_points=0 #do you want to save the data table you are about to generate or no?
...
for index, row in match_results_df.iterrows():
    #if you have not put the match week information in the new data frame yet
...
##### save the match week league points data frame as a csv if you like #####
if save_mw_league_points:
    csv_out_name='data/' + season_span + "_PL_league_points_data.csv"
    print( 'saving data as: ' + csv_out_name )
    league_scores_df.to_csv(csv_out_name, index=False)
